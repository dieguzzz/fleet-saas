...
export async function TripExpensesList({ tripId, orgId }: TripExpensesListProps) {
  ...
  if (error) {
    return <div className="text-red-500">Error loading expenses: {error}</div>;
  }
  ...
  return (
    <div className="bg-white rounded-lg shadow overflow-hidden mt-6">
      <div className="p-4 border-b border-gray-200 flex justify-between items-center bg-gray-50">
        <h3 className="text-md font-semibold text-gray-800">Trip Expenses</h3>
        <Link
          href={`/org/${orgId}/trips/${tripId}/expenses/new`}
            ...
            <tbody className="divide-y divide-gray-200">
              {expenses.map((expense) => (
                <tr key={expense.id} className="hover:bg-gray-50 transition-colors">
                  <td className="px-6 py-3 font-medium text-gray-900 capitalize">{expense.category}</td>
