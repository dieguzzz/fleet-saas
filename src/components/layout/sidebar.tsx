...
const getNavSections = (orgSlug: string, orgType: string = 'fleet'): NavSection[] => {
  ...
  // Operaciones — shared + org-type-specific items
  ...
  sections.push({ label: 'Operaciones', items: operacionesItems });
...
interface SidebarProps {
  isOpen: boolean;
  onClose: () => void;
  sidebarProgress: MotionValue<number>;
}

export function Sidebar({ isOpen, onClose, sidebarProgress }: SidebarProps) {
  ...
  const sections = getNavSections(org.slug, org.org_type);
    ...
    <>
      ...
      >
        ...
        <div className="h-14 px-4 border-b border-sidebar-border flex items-center justify-between shrink-0">
          ...
          <button
            onClick={() => { haptic(); onClose(); }}
            className="lg:hidden p-2 rounded-xl text-sidebar-foreground/40 hover:text-sidebar-foreground hover:bg-sidebar-accent transition-colors shrink-0 min-w-[44px] min-h-[44px] flex items-center justify-center"
                    ...
                    return (
                      <li key={item.href}>
                        <Link
                          ...
                          onClick={() => { haptic(); onClose(); }}
                          className={cn(
                            'flex items-center gap-3 px-3 rounded-xl text-sm transition-colors',
        ...
        <div className="p-3 border-t border-sidebar-border space-y-1">
          ...
          <button
            ...
            disabled={isPending}
            className="w-full flex items-center gap-2 px-3 min-h-[44px] rounded-xl text-xs text-sidebar-foreground/50 hover:text-red-400 hover:bg-sidebar-accent transition-colors"
          >
            <svg className="size-4 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
